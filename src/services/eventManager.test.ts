...
tap.test( "Testing the parser.", async t => {
    ...
    await t.test( "Message is wrong", async t => {
        ...
        t.notOk( ( await service.parseEvent( "0", `[description]\nTesting\n[time]\n<t:${ DateTime.now().plus( { days: 1 } ).toUnixInteger() }:f>` ) ).isValid, "Bad event name" )
        t.notOk( ( await service.parseEvent( "0", `[announcement]\nHello World\n[time]\n<t:${ DateTime.now().plus( { days: 1 } ).toUnixInteger() }:f>` ) ).isValid, "Bad event description" )
        t.notOk( ( await service.parseEvent( "0", `[announcement]\nHello World\n[description]\nTesting` ) ).isValid, "Bad time" )
        t.notOk( ( await service.parseEvent( "0", `[announcement]\nHello World\n[description]\nTesting\n[time]\n<t:${ DateTime.now().toUnixInteger() }:f>` ) ).isValid, "Time is before now" );
