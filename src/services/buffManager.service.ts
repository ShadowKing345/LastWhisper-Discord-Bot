...
import {
  ...
  MessageSettings,
  Week,
} from "../models/buff_manager/index.js";
...
export class BuffManagerService {
  ...
  public async tryGetConfig(
    interaction: CommandInteraction
  ): Promise<[BuffManagerConfig, boolean]> {
    ...
    if (
      config.weeks.filter((week) => !("isEnabled" in week) || week.isEnabled)
        .length <= 0
    ) {
      await interaction.reply({
  ...
  public async postDailyMessage(client: Client): Promise<void> {
    ...
    const configs: BuffManagerConfig[] = (
      await this.buffManagerConfigRepository.getAll()
    ).filter(
      (config) =>
        client.guilds.cache.has(config.guildId) && config.buffs.length > 0
    );
    ...
    for (const config of configs) {
      try {
        ...
        if (
          !now.hasSame(
            DateTime.fromFormat(messageSettings.hour, "HH:mm"),
            "minute"
          )
        )
          continue;
        ...
        const channel: TextChannel | null = (await client.channels.fetch(
          messageSettings.channelId
        )) as TextChannel | null;
        if (!(channel?.isTextBased && channel.guildId === config.guildId)) {
          this.logger.warn(
            `Invalid channel messageSettings.channelId  ID for guild config.guildId. ${skipping}...`
          );
          continue;
        }
        ...
        if (!buff) {
          this.logger.warn(
            `Invalid buff ID buffId for guild config.guildId. ${skipping}...`
          );
          continue;
        }
        ...
        if (
          messageSettings.dow !== null &&
          messageSettings.dow === now.weekday
        ) {
          this.logger.debug(`Posting week message.`);
          await channel.send({
            embeds: [
              this.createWeekEmbed(
                messageSettings.weekMessage,
                week,
                config.buffs,
                now
              ),
